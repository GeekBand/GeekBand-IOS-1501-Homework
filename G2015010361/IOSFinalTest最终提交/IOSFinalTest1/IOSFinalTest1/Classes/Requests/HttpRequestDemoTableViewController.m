...
void DemoRequestHanlder(WBHttpRequest *httpRequest, id result, NSError *error)
{
    ...
    if ([[result class] isSubclassOfClass:[WeiboUser class]]) {
        WeiboUser *aUser = result;
        aString1 = aUser.userID;
        aString2 = aUser.name;
        aString3 = aUser.province;
        aString4 = aUser.city;
        aString5 = aUser.gender;
    }else if ([[result class] isSubclassOfClass:[NSDictionary class]]) {
        followerArray = [result valueForKey:@"users"];
        follow0 = [followerArray objectAtIndex:0];
        follow1 = [followerArray objectAtIndex:1];
        follow2 = [followerArray objectAtIndex:2];
        follow3 = [followerArray objectAtIndex:3];
        follow4 = [followerArray objectAtIndex:4];
        aString1 = follow0.name;
        aString2 = follow1.name;
        aString3 = follow2.name;
        aString4 = follow3.name;
        aString5 = follow4.name;
    ...
    else
    {
        ...
        title = NSLocalizedString(@"收到网络回调\n打印返回值到输出台\n在此处打印部分解析到的数据", nil);
        alert = [[UIAlertView alloc] initWithTitle:title
                                           message:[NSString stringWithFormat:@"ID:%@\n姓名昵称:%@\n省代码:%@\n市代码:%@\n性别:%@\n",aString1, aString2, aString3,aString4,aString5]
                                          delegate:nil
                                 cancelButtonTitle:NSLocalizedString(@"确定", nil)
                                 otherButtonTitles:nil];
        NSLog(@"%@",result);
            ...
            title = NSLocalizedString(@"收到网络回调\n打印返回值到输出台\n在此处打印部分解析到的数据", nil);
            alert = [[UIAlertView alloc] initWithTitle:title
                                               message:[NSString stringWithFormat:@"五名最活跃好友：\n%@\n%@\n%@\n%@\n%@\n",aString1, aString2, aString3,aString4,aString5]
                                              delegate:nil
                                     cancelButtonTitle:NSLocalizedString(@"确定", nil)
                                     otherButtonTitles:nil];
            NSLog(@"%@",result);
...
void DemoRequestHanlder2(WBHttpRequest *httpRequest, id result, NSError *error)
{
    NSString *title = nil;
    UIAlertView *alert = nil;
    ...
    else
    {
            ...
            title = NSLocalizedString(@"收到网络回调\n打印返回值到输出台\n在此处打印部分解析到的数据", nil);
            alert = [[UIAlertView alloc] initWithTitle:title
                                               message:[NSString stringWithFormat:@"存入到follow plist:%@",result]
                                              delegate:nil
                                     cancelButtonTitle:NSLocalizedString(@"确定", nil)
                                     otherButtonTitles:nil];
        
        NSDictionary *dictionary = result;
        NSString *dicPath = [BLUtility getPathWithinDocumentDir:@"follower.plist"];
        [dictionary writeToFile:dicPath atomically:YES];
        
        NSDictionary *cachedDic = [NSDictionary dictionaryWithContentsOfFile:dicPath];
        NSLog(@"存入document的follow list:%@", cachedDic);
...
//            break;
        case SaveFollowDataToDocuments:
            cell.textLabel.text = @"第10题，存入粉丝信息到Documents";
            break;
        default:
            cell.textLabel.text = @"";
            break;
...
- (void)saveFollowerData
{
    ...
    [WBHttpRequest requestForFollowersListOfUser:myDelegate.wbCurrentUserID withAccessToken:myDelegate.wbtoken andOtherProperties:nil queue:nil withCompletionHandler:^(WBHttpRequest *httpRequest, id result, NSError *error) {
        DemoRequestHanlder2(httpRequest, result, error);
    }];
