...
@interface GWHomeTableViewController () <UITableViewDataSource,
                                         UITableViewDelegate>
...
- (void)downloadData {
  ...
  NSURL *url = [NSURL URLWithString:@"http://ax.itunes.apple.com/WebObjects/"
                      @"MZStoreServices.woa/ws/RSS/"
  ...
  [configuration
      setAllowsCellularAccess:[GWGlobal shareGlobal].useCellularFlag];
      ...
      downloadTaskWithURL:url
        completionHandler:^(NSURL *location, NSURLResponse *response,
          ...
          if (httpResponse.statusCode == 200) {
            ...
            NSDictionary *appListRequests =
                [NSJSONSerialization JSONObjectWithData:jsonRequests
                                                options:0
                                                  error:NULL];
  ...
  //
  //    NSDictionary *appListRequests = [NSJSONSerialization
  //    JSONObjectWithData:jsonRequests options:0 error:NULL];
  //
  //    self.appEntries = [appListRequests valueForKeyPath:@"feed.entry"];
}
  ...
  if (status == NotReachable) {
    ...
    if ([GWGlobal shareGlobal].useCellularFlag) {
      ...
      UIAlertView *alert = [[UIAlertView alloc]
              initWithTitle:nil
                    message:@"Download failed, please check your WIFI is on. "
                    @"Or go to Settings > Generial turn on \"Use "
                    @"Cellular\".)"
                   delegate:nil
          cancelButtonTitle:@"Get it."
          otherButtonTitles:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  GWAppsTableViewCell *cell = (GWAppsTableViewCell *)
      [tableView dequeueReusableCellWithIdentifier:@"reuseIdentifierApps"];
...
completed:
  ...
  // Lazy load image
  //    [cell.appImage setImageWithURL:[NSURL URLWithString:[mediumImage
  //    valueForKeyPath:@"label"]] placeholderImage:[UIImage
  //    imageNamed:@"placeholder"]];
  //

  //  cell.appImage.image = [UIImage imageNamed:@"placeholder"];
  //  dispatch_queue_t queue = dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0);
  //  dispatch_async(queue, ^{
  //    NSData *data = [NSData dataWithContentsOfURL:[NSURL
  //    URLWithString:[mediumImage valueForKeyPath:@"label"]]];
  ...
  [q addOperationWithBlock:^{
    NSData *data = [NSData
        dataWithContentsOfURL:
            [NSURL URLWithString:[mediumImage valueForKeyPath:@"label"]]];
    cell.appImage.image = [UIImage imageWithData:data];
  }];
...
- (void)configureAppsCell:(GWAppsTableViewCell *)cell forIndexPath:(NSIndexPath
*)indexPath {
    ...
    NSArray *imimage = [emtry valueForKeyPath:@"im:image"];
    NSDictionary *mediumImage = imimage[1];
    UIImage *image = [UIImage imageWithData:[mediumImage
valueForKeyPath:@"label"]];
